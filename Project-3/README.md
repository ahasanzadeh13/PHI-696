...
![Project3Q5](https://user-images.githubusercontent.com/106703400/223296934-5b8fa0bc-2f10-4b9a-a680-49a1164ce40a.png)
...
On the other hand, Disposition is a realizable entity. The book has the disposition of being used as a "table wobble-stopper" the same way that a chair has the disposition of "being used as a weapon to smack someone over the head with." Dispositions are realizable entities, meaning that they are acquired depending on the physicality and the situation of use.
  ...
  (h) Spacetime is a class in BFO.
    False. I say false, just because it is technicaly Spatiotemporal Region that is a BFO class.
  (i) The continuant fiat boundary class of BFO is closed, meaning, there are no subclasses beyond those identified presently in BFO. 
    True. This is with the assumption that we are talking about adding subclasses in BFO. It might be the case that a field-specific ontology would want to add subclasses.
```
...
```
  ChatGPT helped with making the explanation simpler:
  Imagine you have a really big puzzle that you need to solve. Instead of trying to solve the whole puzzle at once, you can solve small parts of it at a time. This makes it much easier and faster to solve the puzzle.
  Similarly, when people make changes to a big computer system called an ontology, they don't want to have to solve the whole system again every time. So instead, they use a special tool called the Pellet reasoner that can solve small parts of the system at a time, only looking at the parts that have changed. This makes it much faster and easier to update the system when changes are made.
```
